...
function init() {
  ...
  var _Object$assign = Object.assign({}, defaultOptions, options),
      wrapperSelector = _Object$assign.wrapperSelector,
      stickySelector = _Object$assign.stickySelector,
      inputSelector = _Object$assign.inputSelector,
      gridSelector = _Object$assign.gridSelector,
      gridItemSelector = _Object$assign.gridItemSelector,
      columnSizerSelector = _Object$assign.columnSizerSelector,
      gutterSizerSelector = _Object$assign.gutterSizerSelector,
      searchTextSelectors = _Object$assign.searchTextSelectors,
      topOffset = _Object$assign.topOffset,
      layoutOnImagesLoaded = _Object$assign.layoutOnImagesLoaded,
      filteringClass = _Object$assign.filteringClass,
      transitionDuration = _Object$assign.transitionDuration;

  var $stickyWrapper = jquery__WEBPACK_IMPORTED_MODULE_0___default()(wrapperSelector),
      $input = jquery__WEBPACK_IMPORTED_MODULE_0___default()(inputSelector),
      $grid = jquery__WEBPACK_IMPORTED_MODULE_0___default()(gridSelector),
      $items = jquery__WEBPACK_IMPORTED_MODULE_0___default()(gridItemSelector); // Sticky search bar

  Object(stickybits__WEBPACK_IMPORTED_MODULE_1__["default"])(stickySelector, {
    stickyBitStickyOffset: topOffset
  }); // Initialize Isotope layout and sort data

  var isotope = new isotope_layout__WEBPACK_IMPORTED_MODULE_2___default.a(gridSelector, {
  ...
  if (layoutOnImagesLoaded) imagesloaded__WEBPACK_IMPORTED_MODULE_3___default()(gridSelector, function () {
    isotope.layout();
  }); // Scroll to wrapper top

  var isScrollingToWrapperTop = false;

  function scrollToWrapperTop() {
    if (isScrollingToWrapperTop) return;
    var scrollTarget = Math.round($stickyWrapper.offset().top - topOffset);
    if (jquery__WEBPACK_IMPORTED_MODULE_0___default()('html').scrollTop() == scrollTarget) return;
  ...
  var fuseList = $items.get().map(function (item, itemIndex) {
    var fuseEntry = {
      id: itemIndex
    };
    searchTextSelectors.forEach(function (selector, selectorIndex) {
      fuseEntry[selectorIndex.toString()] = jquery__WEBPACK_IMPORTED_MODULE_0___default()(item).find(selector.selector || selector).text();
    });
    return fuseEntry;
  });
  var fuseKeys = searchTextSelectors.map(function (selector, index) {
  ...
  var fuse = new fuse_js__WEBPACK_IMPORTED_MODULE_4___default.a(fuseList, fuseOptions); // Score items by search

  function scoreBySearch(text) {
    var scores = fuse.search(text).reduce(function (object, item) {
      object[item.item] = item.score;
      return object;
    }, {});
    $items.each(function (index, item) {
      var score = scores[index.toString()] || 1;
      jquery__WEBPACK_IMPORTED_MODULE_0___default()(item).attr('isotope-search-score', score);
    });
  ...
  function scoreByIndex() {
    $items.each(function (index, item) {
      jquery__WEBPACK_IMPORTED_MODULE_0___default()(item).attr('isotope-search-score', index * 0.00001);
    });
  ...
  $input.on('change keyup paste', function () {
    scrollToWrapperTop();
    var cleanedSearch = $input.val().replace(/\s+/g, ' ').trim();
